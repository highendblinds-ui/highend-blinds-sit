...
<section id="contact" class="hb-sec hb-contact" style="padding:48px 0 72px">
	<div class="hb-wrap" style="max-width:1160px">
		<div class="hb-form">
			...
			<?php
			/* If a form plugin shortcode is set in the Customizer, render it; otherwise show the static form. */
			$form_sc = get_theme_mod( 'highend_form_shortcode', '' );
			if ( $form_sc ) {
				echo do_shortcode( $form_sc );
			} elseif ( isset( $_GET['estimate_sent'] ) ) {
				echo '<div style="background:#EAF7EE;border:1px solid #BFE6C9;color:#1e5c31;border-radius:12px;padding:16px;font-size:14px;font-weight:600">Thanks! Your request has been sent — we\'ll be in touch shortly.</div>';
			} else {
				?>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" novalidate>
					...
					<textarea name="message" rows="4" aria-label="Message" placeholder="Message"></textarea>
					<button class="btn btn--primary" type="submit" style="width:100%;justify-content:center">Submit</button>
					<p class="note">We respect your privacy. Your details are only used to prepare your estimate.</p>
				</form>
				<?php
			}
			?>
		...
		<div>
			...
			<div class="hb-cinfo">
				...
				<div class="line"><span class="ico"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="12" r="9"></circle><path d="M12 7v5l3 2"></path></svg></span><div style="line-height:1.7">Mon &ndash; Fri: 9:00 AM &ndash; 6:00 PM<br>Sat: 12:00 PM &ndash; 4:00 PM<br>Sun: Closed</div></div>
			</div>
			<div class="hb-map">
